...
void LPI2C_MasterSetBaudRate(LPI2C_Type *base, uint32_t sourceClock_Hz, uint32_t baudRate_Hz)
{
    ...
    */
    uint8_t tmpHigh = (uint8_t)((bestclkCycle - (2U + filtScl) / bestDivider) / 2U);
    while (tmpHigh > (bestclkCycle - 52U * sourceClock_Hz / baudRate_Hz / bestDivider / 100U + 1U))
    {
        tmpHigh = (uint8_t)(tmpHigh - 1U);
    }
    ...
    uint8_t tmpHold = (uint8_t)((uint8_t)(sourceClock_Hz / baudRate_Hz / bestDivider / 2U) - 1U);
    ...
    uint8_t tmpDataVd = (uint8_t)((uint8_t)(sourceClock_Hz / baudRate_Hz / bestDivider / 4U) - 1U);
    ...
    if ((sourceClock_Hz / baudRate_Hz / 20U) > (bestDivider + 2U))
    {
        ...
        {
            filtSda = (uint8_t)((uint8_t)(sourceClock_Hz / baudRate_Hz / 20U) - bestDivider - 2U);
        }
                                                  ...
                                                  lpi2c_state_machine_param_t *stateParams)
{
    ...
    sendval    = (uint16_t)(((uint16_t)handle->buf[0]) | (((uint16_t)handle->buf[1]) << 8U));
    base->MTDR = sendval;
    handle->buf++;
    handle->buf++;
