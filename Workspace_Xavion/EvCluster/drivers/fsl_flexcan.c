...
static bool FLEXCAN_IsMbOccupied(CAN_Type *base, uint8_t mbIdx)
{
    ...
    if (0U != (base->MCR & CAN_MCR_RFEN_MASK))
    {
        ...
        lastOccupiedMb = (uint8_t)(((lastOccupiedMb + 1) * 2) + 5);
                                ...
                                flexcan_timing_config_t *pTimingConfig)
{
    ...
    pTimingConfig->phaseSeg1 = pTimingConfig->phaseSeg1 - 1;
    pTimingConfig->phaseSeg2 = pTimingConfig->phaseSeg2 - 1;
    pTimingConfig->propSeg = pTimingConfig->propSeg -1;
    pTimingConfig->rJumpwidth = pTimingConfig->rJumpwidth -1;
}
...
static void FLEXCAN_FDGetSegments(uint32_t bitRateFD, uint32_t tqNum, flexcan_timing_config_t *pTimingConfig)
{
    ...
    pTimingConfig->fphaseSeg1 = pTimingConfig->fphaseSeg1 -1;
    pTimingConfig->fphaseSeg2 = pTimingConfig->fphaseSeg2 - 1;
    pTimingConfig->frJumpwidth = pTimingConfig->frJumpwidth - 1;
}
                                                         ...
                                                         flexcan_timing_config_t *pTimingConfig)
{
    ...
    do
    {
        ...
        configTemp.phaseSeg1 = configTemp.phaseSeg1 -1;
        configTemp.phaseSeg2 = configTemp.phaseSeg2 - 1;
        configTemp.propSeg =  configTemp.propSeg - 1;
        configTemp.rJumpwidth = configTemp.rJumpwidth -1;
...
void FLEXCAN_SetRxFifoConfig(CAN_Type *base, const flexcan_rx_fifo_config_t *pRxFifoConfig, bool enable)
{
    ...
    if (enable)
    {
        ...
        numMbOccupy = (uint8_t)(6U + (rffn + 1U) * 2U);
...
#if (defined(FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER)) && (FSL_FEATURE_FLEXCAN_HAS_EXTENDED_FLAG_REGISTER > 0)
        uint64_t u64flag = 1;
        if (0U == FLEXCAN_GetMbStatusFlags(base, u64flag << mbIdx))
        {
        	status = kStatus_Fail;
        	return status;
        }
