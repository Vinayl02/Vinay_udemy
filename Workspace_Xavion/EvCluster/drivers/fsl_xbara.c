...
void XBARA_ClearStatusFlags(XBARA_Type *base, uint32_t mask)
{
    ...
    regVal  = (uint16_t)(regVal & (uint16_t)(~(XBARA_CTRL0_STS0_MASK | XBARA_CTRL0_STS1_MASK)));
    /* Write 1 to interrupt flag bits corresponding to mask */
    regVal  = (uint16_t)(regVal | (uint16_t)(mask & (XBARA_CTRL0_STS0_MASK | XBARA_CTRL0_STS1_MASK)));
    ...
    regVal = (base->CTRL1);
    /* Perform this command to avoid writing 1 into interrupt flag bits */
    regVal  = (uint16_t)(regVal & (uint16_t)(~(XBARA_CTRL1_STS2_MASK | XBARA_CTRL1_STS3_MASK)));
    /* Write 1 to interrupt flag bits corresponding to mask */
    regVal  = (uint16_t)(regVal | (uint16_t)((mask >> 16U) & (XBARA_CTRL1_STS2_MASK | XBARA_CTRL1_STS3_MASK)));
