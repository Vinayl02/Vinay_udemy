...
static inline uint16_t dutyCycleToReloadValue(uint8_t dutyCyclePercent)
{
    ...
    return (uint16_t)(((65535U * dutyCyclePercent) + 50U) / 100U);
}
...
status_t PWM_Init(PWM_Type *base, pwm_submodule_t subModule, const pwm_config_t *config)
{
     ...
     */
    reg =
    		(uint16_t)(reg & ~(uint16_t)(PWM_CTRL2_CLK_SEL_MASK | PWM_CTRL2_FORCE_SEL_MASK | PWM_CTRL2_INIT_SEL_MASK | PWM_CTRL2_INDEP_MASK |
    ...
    reg = (uint16_t)(reg | (PWM_CTRL2_CLK_SEL(config->clockSource) | PWM_CTRL2_FORCE_SEL(config->forceTrigger) |
    ...
    switch (config->pairOperation)
    {
        ...
        case kPWM_ComplementaryPwmA:
            base->MCTRL = (uint16_t)(base->MCTRL & ~((uint16_t)1U << (PWM_MCTRL_IPOL_SHIFT + (uint16_t)subModule)));
            break;
    ...
    reg = (uint16_t)(reg & ~(uint16_t)(PWM_CTRL_PRSC_MASK | PWM_CTRL_LDFQ_MASK | PWM_CTRL_LDMOD_MASK));
    reg = (uint16_t)(reg | (PWM_CTRL_PRSC(config->prescale) | PWM_CTRL_LDFQ(config->reloadFrequency)));
    ...
    base->MASK = (uint16_t)(base->MASK & ~(uint16_t)(PWM_MASK_MASKX_MASK | PWM_MASK_MASKA_MASK | PWM_MASK_MASKB_MASK));
...
{
    /* Stop the submodule */
    base->MCTRL = (uint16_t)(base->MCTRL & ~((uint16_t)1U << (PWM_MCTRL_RUN_SHIFT + (uint16_t)subModule)));
                      ...
                      uint32_t srcClock_Hz)
{
    ...
    for (i = 0; i < numOfChnls; i++)
    {
        ...
        pwmHighPulse = (uint16_t)((pulseCnt * chnlParams->dutyCyclePercent) / 100U);
        ...
        switch (mode)
        {
            case kPWM_SignedCenterAligned:
                ...
                if (i == 0U)
                {
                    ...
                    base->SM[subModule].VAL1 = (uint16_t)(modulo - 1U);
                }
            ...
            case kPWM_CenterAligned:
                ...
                if (i == 0U)
                {
                    ...
                    /* Indicates the end of the PWM period */
                    /* The change during the end to start of the PWM period requires a count time */
                    base->SM[subModule].VAL1 = (uint16_t)(pulseCnt - 1U);
                }

                /* Setup the PWM dutycycle */
                if (chnlParams->pwmChannel == kPWM_PwmA)
                {
                    base->SM[subModule].VAL2 = (uint16_t)(((pulseCnt - pwmHighPulse) / 2U));
                    base->SM[subModule].VAL3 = (uint16_t)(((pulseCnt + pwmHighPulse) / 2U));
                }
                else
                {
                    base->SM[subModule].VAL4 = (uint16_t)(((pulseCnt - pwmHighPulse) / 2U));
                    base->SM[subModule].VAL5 = (uint16_t)(((pulseCnt + pwmHighPulse) / 2U));
                }
                break;
        ...
        {
            case kPWM_PwmA:
                base->SM[subModule].OCTRL = (uint16_t)(base->SM[subModule].OCTRL & ~((uint16_t)PWM_OCTRL_PWMAFS_MASK));
                base->SM[subModule].OCTRL = (uint16_t)(base->SM[subModule].OCTRL | (((uint16_t)(chnlParams->faultState) << (uint16_t)PWM_OCTRL_PWMAFS_SHIFT) &
                                              (uint16_t)PWM_OCTRL_PWMAFS_MASK));
                break;
            case kPWM_PwmB:
                base->SM[subModule].OCTRL = (uint16_t)(base->SM[subModule].OCTRL & ~((uint16_t)PWM_OCTRL_PWMBFS_MASK));
                base->SM[subModule].OCTRL = (uint16_t)(base->SM[subModule].OCTRL | (((uint16_t)(chnlParams->faultState) << (uint16_t)PWM_OCTRL_PWMBFS_SHIFT) &
        ...
        {
            base->SM[subModule].OCTRL = (uint16_t)(base->SM[subModule].OCTRL & ~((uint16_t)1U << (uint16_t)polarityShift));
        }
        else
        {
            base->SM[subModule].OCTRL= (uint16_t)(base->SM[subModule].OCTRL |((uint16_t)1U << (uint16_t)polarityShift));
        ...
        {
            /* Enable PWM output */
            base->OUTEN = (uint16_t) (base->OUTEN | ((uint16_t)1U << ((uint16_t)outputEnableShift + (uint16_t)subModule)));
        }
                                ...
                                bool doSync)
{
    ...
    if (pwmChannel != kPWM_PwmX)
    {
        ...
        {
            base->MCTRL = (uint16_t)(base->MCTRL |(uint16_t)(PWM_MCTRL_CLDOK(1UL << (uint8_t)subModule)));
        }
        ...
        base->SM[subModule].CTRL = base->SM[subModule].CTRL |PWM_CTRL_LDMOD_MASK;

        /* phase shift value */
        shift = (uint16_t)((pulseCnt * shiftvalue) / 100U);
        ...
        if (pwmChannel == kPWM_PwmA)
        {
            ...
            base->SM[subModule].VAL3 = (uint16_t)((uint32_t)PWM_GetComplementU16(modulo) + (uint32_t)pwmHighPulse + (uint32_t)shift -1U);
        }
        else if (pwmChannel == kPWM_PwmB)
        {
            base->SM[subModule].VAL4 = PWM_GetComplementU16(modulo) + shift;
            base->SM[subModule].VAL5 = (uint16_t)((uint32_t)PWM_GetComplementU16(modulo) +(uint32_t)pwmHighPulse + (uint32_t)shift - 1U);
        }
        else
        {
            return kStatus_Fail;
        ...
        if (doSync)
        {
            ...
            base->MCTRL =  (uint16_t)(base->MCTRL | (uint16_t)(PWM_MCTRL_LDOK(1UL << (uint8_t)subModule)));
        }
    }
    else
    {
                           ...
                           const pwm_input_capture_param_t *inputCaptureParams)
{
    ...
    switch (pwmChannel)
    {
        case kPWM_PwmA:
            ...
            reg = (uint16_t)(PWM_CAPTCTRLA_INP_SELA(inputCaptureParams->captureInputSel) |
            ...
            base->OUTEN =(uint16_t)(base->OUTEN & ~((uint16_t)1U << (PWM_OUTEN_PWMA_EN_SHIFT + (uint16_t)subModule)));
            break;
        case kPWM_PwmB:
            /* Setup the capture paramters for PWM B pin */
            reg = (uint16_t)(PWM_CAPTCTRLB_INP_SELB(inputCaptureParams->captureInputSel) |
            ...
            base->OUTEN = (uint16_t)(base->OUTEN &~((uint16_t)1U << (PWM_OUTEN_PWMB_EN_SHIFT + (uint16_t)subModule)));
            break;
        case kPWM_PwmX:
            reg = (uint16_t)(PWM_CAPTCTRLX_INP_SELX(inputCaptureParams->captureInputSel) |
            ...
            base->OUTEN = (uint16_t)(base->OUTEN & ~((uint16_t)1U << (PWM_OUTEN_PWMX_EN_SHIFT + (uint16_t)subModule)));
            break;
        default:
            assert(false);
            break;
...
void PWM_SetupFaultInputFilter(PWM_Type *base, const pwm_fault_input_filter_param_t *faultInputFilterParams)
{
    ...
    {
        base->FFILT = (uint16_t)(base->FFILT & ~(uint16_t)(PWM_FFILT_FILT_PER_MASK));
    }
...
void PWM_SetupFaults(PWM_Type *base, pwm_fault_input_t faultNum, const pwm_fault_param_t *faultParams)
{
    ...
    {
        reg = (uint16_t)(reg | ((uint16_t)1U << (PWM_FCTRL_FLVL_SHIFT + (uint16_t)faultNum)));
    }
    else
    {
        reg = (uint16_t)(reg & ~((uint16_t)1U << (PWM_FCTRL_FLVL_SHIFT + (uint16_t)faultNum)));
    ...
    if ((uint16_t)faultParams->faultClearingMode != 0U)
    {
        ...
        reg = (uint16_t)(reg & ~((uint16_t)1U << (PWM_FCTRL_FAUTO_SHIFT + (uint16_t)faultNum)));
        if (faultParams->faultClearingMode == kPWM_ManualSafety)
        {
            /* Use manual fault clearing with safety mode enabled */
            reg = (uint16_t)(reg |((uint16_t)1U << (PWM_FCTRL_FSAFE_SHIFT + (uint16_t)faultNum)));
        }
        else
        {
    ...
    if (faultParams->enableCombinationalPath)
    {
        ...
        base->FCTRL2 = (uint16_t)(base->FCTRL2 & ~((uint16_t)1U << (uint16_t)faultNum));
    }
    else
    {
    ...
    reg = base->FSTS;
    reg = (uint16_t)( reg & ~(((uint16_t)1U << (PWM_FSTS_FFULL_SHIFT + (uint16_t)faultNum)) |
    ...
    switch (faultParams->recoverMode)
    {
        ...
        case kPWM_RecoverHalfCycle:
        	reg  = (uint16_t)( reg | ((uint16_t)1U << (PWM_FSTS_FHALF_SHIFT + (uint16_t)faultNum)));
            break;
...

{
    ...
    shift =  (uint16_t)(((uint16_t)subModule * 4U) + ((uint16_t)pwmChannel * 2U));

    /* Setup the signal to be passed upon occurrence of a FORCE_OUT signal */
    reg = base->DTSRCSEL;
    reg = (uint16_t)(reg & ~((uint16_t)0x3U << shift));
    reg = (uint16_t)(reg | (uint16_t)((uint16_t)mode << shift));
    base->DTSRCSEL = reg;
}
...
void PWM_EnableInterrupts(PWM_Type *base, pwm_submodule_t subModule, uint32_t mask)
{
	...
	base->SM[subModule].INTEN = ((uint16_t)(base->SM[subModule].INTEN | ((uint16_t)mask & 0xFFFFU)));
    /* Fault related interrupts */
    base->FCTRL = ((uint16_t)(base->FCTRL | ((uint16_t)(mask >> 16U) & PWM_FCTRL_FIE_MASK)));
}
...
void PWM_ClearStatusFlags(PWM_Type *base, pwm_submodule_t subModule, uint32_t mask)
{
     ...
     */
    reg = (uint16_t)(reg & ~(uint16_t)(PWM_FSTS_FFLAG_MASK));
    reg = (uint16_t)(reg | (uint16_t)((mask >> 16U) & PWM_FSTS_FFLAG_MASK));
    base->FSTS = reg;
}
...
void PWM_SetClockMode(PWM_Type *base, pwm_submodule_t subModule, pwm_clock_prescale_t prescaler)
{
    ...
    if (0U != (base->MCTRL & PWM_MCTRL_LDOK(1UL << (uint8_t)subModule)))
    {
    	 base->MCTRL = (uint16_t)(base->MCTRL | PWM_MCTRL_CLDOK(1UL << (uint8_t)subModule));
    }
    /* Set submodule prescaler. */
    reg = (uint16_t)(reg & ~(uint16_t)PWM_CTRL_PRSC_MASK);
    reg  = (uint16_t)(reg | PWM_CTRL_PRSC(prescaler));
    base->SM[subModule].CTRL = reg;
    /* Set Load mode to make Buffered registers take effect immediately when LDOK bit set */
    base->SM[subModule].CTRL = (uint16_t)(base->SM[subModule].CTRL | PWM_CTRL_LDMOD_MASK);
    /* Set LDOK bit to load buffer registers */
    base->MCTRL = (uint16_t)(base->MCTRL | PWM_MCTRL_LDOK(1UL << (uint8_t)subModule));
    /* Restore Load mode */
    base->SM[subModule].CTRL = reg;
}
...
void PWM_SetPwmForceOutputToZero(PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwmChannel, bool forcetozero)
{
    ...
    else
    {
    	...
    	base->MASK = (uint16_t)(base->MASK & ~mask);
    }

    /* Select local force signal */
    base->SM[subModule].CTRL2 = (uint16_t)(base->SM[subModule].CTRL2 & ~(uint16_t)PWM_CTRL2_FORCE_SEL_MASK);
                          ...
                          pwm_output_state_t outputstate)
{
    ...
    if (kPWM_PwmA == pwmChannel)
    {
        ...
        sourceShift = (uint16_t)(PWM_DTSRCSEL_SM0SEL23_SHIFT + ((uint16_t)subModule * 4U));
    }
    else if (kPWM_PwmB == pwmChannel)
    {
        mask        = PWM_MASK_MASKB(0x01UL << (uint8_t)subModule);
    ...
    else
    {
        ...
        if (kPWM_PwmX != pwmChannel)
        {
            if (kPWM_HighState == outputstate)
            {
                base->SWCOUT |= swcout;
                base->DTSRCSEL =
                		(uint16_t)(base->DTSRCSEL & ~(uint16_t)(0x3UL << sourceShift)) | (uint16_t)(0x2UL << sourceShift);
            }
            else if (kPWM_LowState == outputstate)
            {
            	base->SWCOUT = (uint16_t)(base->SWCOUT & ~swcout);
                base->DTSRCSEL =
                		(uint16_t)(base->DTSRCSEL & ~(uint16_t)(0x3UL << sourceShift)) | (uint16_t)(0x2UL << sourceShift);
            }
            else if (kPWM_NormalState == outputstate)
            {
            	base->DTSRCSEL = (uint16_t)(base->DTSRCSEL & ~(uint16_t)(0x3UL << sourceShift));
            }
            else
            {
                base->DTSRCSEL =
                		(uint16_t)(base->DTSRCSEL & ~(uint16_t)(0x3UL << sourceShift)) | (uint16_t)(0x1UL << sourceShift);
            }
        }
    }
    ...
    base->SM[subModule].CTRL2 = (uint16_t)(base->SM[subModule].CTRL2 & ~(uint16_t)PWM_CTRL2_FORCE_SEL_MASK);
    /* Issue a local Force trigger event */
    base->SM[subModule].CTRL2 = base->SM[subModule].CTRL2 | PWM_CTRL2_FORCE_MASK;
    /* Restore the source of FORCE OUTPUT signal */
    base->SM[subModule].CTRL2 = reg;
}
