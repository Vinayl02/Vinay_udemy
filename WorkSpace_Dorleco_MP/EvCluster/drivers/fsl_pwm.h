                                               ...
                                               pwm_watermark_control_t pwm_watermark_control)
{
    ...
    {
    	reg = ((uint16_t)(reg & ~((uint16_t)PWM_DMAEN_FAND_MASK)));
    }
    else
    {
    	reg = ((uint16_t)(reg | ((uint16_t)PWM_DMAEN_FAND_MASK)));
                                              ...
                                              pwm_dma_source_select_t pwm_dma_source_select)
{
    ...
    reg = ((uint16_t)(reg & ~((uint16_t)PWM_DMAEN_CAPTDE_MASK)));

    reg = ((uint16_t)(reg | (((uint16_t)pwm_dma_source_select << (uint16_t)PWM_DMAEN_CAPTDE_SHIFT) &
    						   (uint16_t)PWM_DMAEN_CAPTDE_MASK)));
...
static inline void PWM_EnableDMAWrite(PWM_Type *base, pwm_submodule_t subModule, bool activate)
{
    ...
    if (activate)
    {
    	reg = ((uint16_t)(reg | ((uint16_t)PWM_DMAEN_VALDE_MASK)));
    }
    else
    {
    	reg =  ((uint16_t)(reg & (~((uint16_t)PWM_DMAEN_VALDE_MASK))));
...
{
	base->MCTRL = ((uint16_t)(base->MCTRL | PWM_MCTRL_RUN(subModulesToStart)));
}
...
{
	base->MCTRL = ((uint16_t)(base->MCTRL & ~(PWM_MCTRL_RUN(subModulesToStop))));
}
                                           ...
                                           bool activate)
{
    if (activate)
    {
    	base->SM[subModule].TCTRL = ((uint16_t)(base->SM[subModule].TCTRL | ((uint16_t)1U << (uint16_t)valueRegister)));
    }
    else
    {
    	base->SM[subModule].TCTRL =  ((uint16_t)(base->SM[subModule].TCTRL & ~((uint16_t)1U << (uint16_t)valueRegister)));
...
{
	base->SM[subModule].TCTRL =((uint16_t) ( base->SM[subModule].TCTRL & ~(PWM_TCTRL_OUT_TRIG_EN_MASK & (valueRegisterMask))));
}
...
static inline void PWM_SetupSwCtrlOut(PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwmChannel, bool value)
{
    if (value)
    {
    	 base->SWCOUT = ((uint16_t) (base->SWCOUT |
...
static inline void PWM_SetPwmLdok(PWM_Type *base, uint8_t subModulesToUpdate, bool value)
{
    if (value)
    {
        base->MCTRL = (uint16_t) (base->MCTRL | PWM_MCTRL_LDOK(subModulesToUpdate));
    }
    else
    {
        base->MCTRL = (uint16_t) (base->MCTRL | PWM_MCTRL_CLDOK(subModulesToUpdate));
    }
}
                                        ...
                                        pwm_fault_state_t faultState)
{
    ...
    {
        case kPWM_PwmA:
        	 reg =(uint16_t)(reg  & ~((uint16_t)PWM_OCTRL_PWMAFS_MASK));
            reg =(uint16_t)(reg  | (((uint16_t)faultState << (uint16_t)PWM_OCTRL_PWMAFS_SHIFT) & (uint16_t)PWM_OCTRL_PWMAFS_MASK));
            break;
        case kPWM_PwmB:
        	 reg =(uint16_t)(reg  & ~((uint16_t)PWM_OCTRL_PWMBFS_MASK));
            reg =(uint16_t)(reg  | (((uint16_t)faultState << (uint16_t)PWM_OCTRL_PWMBFS_SHIFT) & (uint16_t)PWM_OCTRL_PWMBFS_MASK));
            break;
        case kPWM_PwmX:
        	 reg =(uint16_t)(reg  & ~((uint16_t)PWM_OCTRL_PWMXFS_MASK));
            reg =(uint16_t)(reg  | (((uint16_t)faultState << (uint16_t)PWM_OCTRL_PWMXFS_SHIFT) & (uint16_t)PWM_OCTRL_PWMXFS_MASK));
            break;
        default:
            assert(false);
            break;
                                            ...
                                            uint16_t value)
{
    ...
    {
        case kPWM_PwmA:
        	reg =(uint16_t)(reg & ~((uint16_t)PWM_DISMAP_DIS0A_MASK));
        	 reg =(uint16_t)(reg  | (((uint16_t)(value) << (uint16_t)PWM_DISMAP_DIS0A_SHIFT) & (uint16_t)PWM_DISMAP_DIS0A_MASK));
            break;
        case kPWM_PwmB:
        	reg =(uint16_t)(reg   & ~((uint16_t)PWM_DISMAP_DIS0B_MASK));
        	 reg =(uint16_t)(reg  |(((uint16_t)(value) << (uint16_t)PWM_DISMAP_DIS0B_SHIFT) & (uint16_t)PWM_DISMAP_DIS0B_MASK));
            break;
        case kPWM_PwmX:
        	 reg =(uint16_t)(reg & ~((uint16_t)PWM_DISMAP_DIS0X_MASK));
            reg =(uint16_t)(reg | (((uint16_t)(value) << (uint16_t)PWM_DISMAP_DIS0X_SHIFT) & (uint16_t)PWM_DISMAP_DIS0X_MASK));
            break;
        default:
            assert(false);
            break;
...
static inline void PWM_OutputEnable(PWM_Type *base, pwm_channels_t pwmChannel, pwm_submodule_t subModule)
{
    ...
    {
        case kPWM_PwmA:
        	base->OUTEN = (uint16_t)(base->OUTEN | ((uint16_t)1U << ((uint16_t)PWM_OUTEN_PWMA_EN_SHIFT + (uint16_t)subModule)));
        break;
        case kPWM_PwmB:
        	base->OUTEN = (uint16_t)(base->OUTEN | ((uint16_t)1U << ((uint16_t)PWM_OUTEN_PWMB_EN_SHIFT + (uint16_t)subModule)));
            break;
        case kPWM_PwmX:
        	base->OUTEN = (uint16_t)(base->OUTEN | ((uint16_t)1U << ((uint16_t)PWM_OUTEN_PWMX_EN_SHIFT + (uint16_t)subModule)));
            break;
        default:
            assert(false);
            break;
